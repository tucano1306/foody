...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  let rows;
  if (householdId) {
    rows = await sql`DELETE FROM shopping_trips WHERE id = ${id} AND household_id = ${householdId} RETURNING id`;
  } else {
    rows = await sql`DELETE FROM shopping_trips WHERE id = ${id} AND user_id = ${user.userId} AND household_id IS NULL RETURNING id`;
  }
  if (!rows.length) return notFound();
